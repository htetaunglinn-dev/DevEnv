...
const SideDrawer = () => {
  ...
  return (
    <TooltipProvider>
      <ScrollArea
        className={`${isExpanded ? "w-72" : "w-16"} border-r border-gray-300/50 dark:border-gray-300/10 transition-all duration-300 ease-in-out`}
      >
      <div className="hidden h-[calc(100vh-52px)] flex-col justify-between p-4 md:flex">
        <section id="menu-section">
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  ...
                  >
                    <BiSolidLeftArrow
                      size={20}
                      className="transition dark:text-white rotate-180"
                    />
                  </button>
                </TooltipTrigger>
          ...
          <div className="mt-10 flex flex-col dark:text-white/80">
            <h2 className={`text-xs dark:text-white/60 ${isExpanded ? "block" : "hidden"}`}>Discover</h2>
                ...
                <Link key={item.title} href={item.link}>
                  {!isExpanded ? (
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <li
                          className={`flex cursor-pointer items-center gap-4 rounded-md p-2 hover:bg-gray-200/20 w-fit ${pathname === item.link ? "bg-gray-200/60 dark:bg-gray-100/5" : ""}`}
                        >
                          {item.icon}
                        </li>
                      </TooltipTrigger>
                    ...
                    <li
                      className={`flex cursor-pointer items-center gap-4 rounded-md p-2 hover:bg-gray-200/20 w-full ${pathname === item.link ? "bg-gray-200/60 dark:bg-gray-100/5" : ""}`}
                    >
                      {item.icon}
          ...
          {/* Contribute */}
          <div className="mt-10 flex flex-col dark:text-white/80">
            <h2 className={`text-xs dark:text-white/60 ${isExpanded ? "block" : "hidden"}`}>Contribute</h2>
            <ul className="my-4 flex flex-col gap-2">
