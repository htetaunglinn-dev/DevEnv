...
async function seedPosts(): Promise<void> {
  try {
    ...
    const imageBasePath = path.join(__dirname, '../../../client/assets/imgs');
    ...
    for (const postData of seedData) {
      try {
        ...
        const imagePath = path.join(imageBasePath, postData.imageFileName);
